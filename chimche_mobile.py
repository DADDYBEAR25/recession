...
recession_risk = int((danger_count / total_indicators) * 100)

if danger_count <= 1:
    conclusion = '<span style="color:gray">매우 안전</span>'
elif danger_count == 2:
    conclusion = '<span style="color:gray">안전</span>'
elif danger_count == 3:
    conclusion = '<span style="color:red"><strong>투자 주의 요망</strong></span>'
elif danger_count in [4, 5]:
    conclusion = '<span style="color:red"><strong>침체 진입 주의</strong></span>'
else:
    conclusion = '<span style="color:red"><strong>침체 확정</strong></span>'
...
for name, status in results.items():
    ...
    st.markdown(f"- **{name}** → <span style='color:{color}'><strong>{status}</strong></span>", unsafe_allow_html=True)

st.subheader("종합 진단 결과")
st.markdown(f"### {conclusion}", unsafe_allow_html=True)
st.markdown(f"#### 현재 침체 확률: **{recession_risk}%**")
